// tag::short[]
Данная возможность доступна только при установленном xref:ROOT:requirements-software.adoc#browser[поддерживаемом] офисном пакете для следующих файлов: `.doc`, `.docx`, `.docm`, `.xls`, `.xlsx`, `.xlsm`, `.ppt`, `.pptx`, `.odt` и `.ods`.
// end::short[]
