...
Добавлена возможность xref:dv5@programmer:WebClient:client/authorization.adoc[настроить] страницу входа в {wc}.
...
Добавлена возможность xref:dv5@programmer:WebClient:client/signing-api.adoc[управления] диалогом подписания через API.
...
* xref:dv5@programmer:WebClient:server/data-grid-control.adoc#api[Добавлены] новые методы и события API для контролов с табличным UI.
* Был xref:dv5@programmer:WebClient:how-to-use-rest.adoc[добавлен REST Web-API], доступный на стороне клиента {wc}а, который может вызывать функции {dv} API. REST API предназначается для работы из клиентских скриптов.
