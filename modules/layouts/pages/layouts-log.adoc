...
Для программы {kvr} предусмотрен журнал работы. Журнал хранится по пути `C:/Program Files (x86)/Docsvision/WebClientToolkit/Logs/`. Имя файла журнала имеет следующий формат: `WebLayoutsDesigner PID \#\### ГГГГ-ММ-ДД`, где `\####` -- номер процесса.
...
. Найдите конфигурационный файл, расположенный в папке по адресу `C:/Program Files (x86)/Docsvision/WebClientToolkit/WebLayoutsDesigner.appsettings.json`.
...
<.> Сообщения уровней от `Trace` до `Info` из библиотек, название которых начинается с `Microsoft.`, будут записываться в консоль.
<.> Все сообщения уровней от `Error` и выше сообщения будут записываться в файл.
