...
Чтобы определить набор отображаемых способов, в {wc}е создать экземпляр `AuthenticationExtensionFactory` и через него запрашивать доступные для определённой БД расширения аутентификации. Подробнее см. xref:dv5@programmer:WebClient:client/authorization.adoc[].
