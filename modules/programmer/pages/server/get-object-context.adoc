...
+
WARNING: Не используйте метод `GetOrCreateApplicationPoolSessionContext`, это приведет к блокировкам потоков под нагрузкой. В первую очередь нужно использовать метод `GetOrCreateCurrentSessionContext`.
