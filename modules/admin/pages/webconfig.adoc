...
Конфигурационный файл {wc}а располагается по адресу `{webconfig-win}`.
...
include::session-recreation.adoc[tags=webconfig]

include::session-recreation.adoc[tags=webconfig-1]

Подробнее см. "xref:session-recreation.adoc[]".
...
[#hide-user-avatars]
== Скрытие фото сотрудников

.Чтобы скрыть фотографии сотрудников:
. Откройте конфигурационный файл `{webconfig-win}`.
. Перейдите к секции {setgroups-wc}.
. Измените значение параметра *_HideUserAvatars_*:
+
[source]
----
<SettingGroup Name="WebClient">
    <Setting Name="HideUserAvatars" Value="true" /> <.>
</SettingGroup>
----
<.> `HideUserAvatars`:
+
* *_false_* -- фотографии сотрудников не скрываются (значение по умолчанию).
* *_true_* -- фотографии сотрудников скрыты, во всех элементах интерфейса вместо фотографий сотрудников отображаются xref:layouts:guide-layouts-user-profile.adoc#edit[изображения по умолчанию].
...
[#auth-ad]
=== Автоматическая подстановка домена при входе

include::default-domain.adoc[tags=webconfig]

Подробнее см. "xref:default-domain.adoc[]".
