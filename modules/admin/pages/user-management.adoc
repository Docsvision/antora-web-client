...
Пользователи {wc}а должны быть добавлены в системную группу *{dv-users-serv}* xref:new-admin-console@mgmtconsole:servers:appserver-access.adoc#groups[{of-mc}], xref:6.1@platform:admin:config-platform.adoc[конфигурационном файле] или xref:user:directories/staff/groups-employees.adoc[справочнике сотрудников].
