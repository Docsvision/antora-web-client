...
// tag::webconfig[]
Протоколирование запросов сервера {wc}а настраивается в конфигурационном файле модуля по адресу `{webconfig-lin}`, за настройку отвечает параметр `W3CLogging`:
