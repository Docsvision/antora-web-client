...
. При остановке службы Сервера {wc}а в файл журнала {wc}а добавляется запись о завершении {wc}а, например:
+
...
+
. При старте службы Сервера {wc}а создаётся новый файл журнала {wc}а.
...
// tag::webconfig[]
Протоколирование запросов сервера {wc}а настраивается в конфигурационном файле модуля по адресу `{webconfig-lin}` в Linux и `{webconfig-cp}` в Windows, за настройку отвечает параметр `W3CLogging`:
...
Запросы будут записаны в файл `/var/log/docsvision/webclient/webclient-w3c-&#x7b;YYYYMMDD.X&#x7d;.txt` для Linux и `C:\ProgramData\docsvision\webclient\webclient-w3c-&#x7b;YYYYMMDD.X&#x7d;.txt` для Windows. Настройка альтернативного расширения поддерживается только при протоколировании с NLog, см. подробнее <<nlog,ниже>>.

:module-service: сервера {wc}а
