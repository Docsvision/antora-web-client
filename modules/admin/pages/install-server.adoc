...
+
В меню Пуск, в группу {dv} будут добавлены ярлыки {kvr} и WebToolBuilder.
+
...
+
. Откройте {mc} на странице menu:Серверы[Имя сервера > {wc} > Группа настроек "Решения"] выполните импорт стандартных решений для корректной работы
ifdef::kvr-pu[]
{wc}а.
endif::kvr-pu[]
ifndef::kvr-pu[]
