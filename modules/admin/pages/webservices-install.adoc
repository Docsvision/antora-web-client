...
// * Из инсталлятора {wc}а, `{dv} web-client.msi`, компонент _WebToolBuilder_, см. подробнее "xref:6.1@webclient:admin:install-server.adoc[]".
...
image::6.1@webclient:admin:webtoolbuilder.png[Окно программы "WebToolBuilder"]
...
* image:6.1@webclient:layouts:buttons/three-dots.png[Три точки] -- кнопка для выбора манифеста приложения из файловой системы
...
. Перейдите в каталог с WebToolBuilder (по умолчанию `C:\Program Files (x86)\Docsvision\WebClientToolkit\WebToolBuilder\WebToolBuilder.exe`) и запустите утилиту командой с указанием пути к файлу манифеста:
+
 ./WebToolBuilder C:\Program Files (x86)\Docsvision\WebClientToolkit\WebToolBuilder\DVWebTool\DocsVision.DVWebTool.application
+
...
WARNING: Программа _DVWebTool_ работает только в ОС {ms} Windows. Для работы требуется установить ПО {net-r} и {asp-bundle}. ПО можно загрузить с https://dotnet.microsoft.com/en-us/download/dotnet/[сайта Microsoft].
// end::webtool[]

.Чтобы установить "DVWebTool":
. [[install-dvweb]]Чтобы установить _DVWebTool_, необходимо создать образ утилиты. Для этого запустите _WebToolBuilder_ на Windows.
. В поле _Путь к манифесту приложения_ выберите файл манифеста `C:\Program Files (x86)\Docsvision\WebClientToolkit\WebToolBuilder\DVWebTool\DocsVision.DVWebTool.application`. После выбора файла автоматически будут заполнены другие поля.
+
...
image::6.1@webclient:admin:dvweb-final-window.png[Успешное завершение сборки]
+
...
+
После завершения установки программа _DVWebTool_ будет запущена автоматически, а в области уведомлений Windows появится значок программы (image:6.1@webclient:user:buttons/dv-web-ico.png[Две стрелочки в зелёном круге]).
+
...
WARNING: Служба _DVSupService_ работает только в ОС Linux. Для работы требуется установить {asp-bundle}. ПО можно загрузить с https://dotnet.microsoft.com/en-us/download/dotnet/[сайта Microsoft].
...
+
В конфигурационном файле службы по адресу `/usr/lib/docsvision/dvsupservice/appsettings.json` для ОС Linux `C:\Program Files\Docsvision\DvSupService\appsettings.json` для ОС Windows можно указать настройку `WebClientAddress` (по умолчанию null). Если для этой настройки указать адрес сервера (например `\http://docsvision.server.com:5004`), служба будет принимать запросы на открытие только с указанного адреса. При необходимости можно указать несколько адресов.
+
...
image::6.1@webclient:user:r7-notice.png[Диалог сохранения в Р7-Офис]
+
