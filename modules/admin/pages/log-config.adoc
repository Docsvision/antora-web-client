...
+
. Запрос включения журнала с указанием времени работы 1 минута:
+
[subs=attributes]
 http://адрес-сервера-{wc}а:ПОРТ/DiagnosticsLog/SetIntervalTimer?minutes=1
+
По умолчанию, если `SetIntervalTimer` не указывается, таймер работает 1 час.
+
