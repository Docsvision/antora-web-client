...
+
. Запрос включения журнала с указанием времени работы 10 минут:
+
[subs=attributes]
...
При установке любого фильтра используемый таймер будет добавлен в конец текущего периода. Например, повторный запрос без установки таймера мгновенно перезапустит журналирование с действующим таймером. Повторный запрос с установленным таймером (например, 10 минут) добавит в очередь интервал журналирования с указанным таймером (например, 10 минут) после завершения текущего запроса (в общей сложности 20 минут).
====
+
