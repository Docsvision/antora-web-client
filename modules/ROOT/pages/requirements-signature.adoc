...
[NOTE]
====
Дополнительная информация по настройке подписания и шифрования приведена в разделе "xref:5.5.5@backoffice:admin:system-settings.adoc#signature-cypher[Руководстве администратора модуля {bo}]".
====
+
...
. https://cryptopro.ru/products/csp/downloads[КриптоПро CSP {csp-v}] с соответствующей лицензией для проверки и формирования подписей.
...
. Требуются компоненты КриптоПро CSP {csp-v}.
...
. Требуется установить Компоненты КриптоПро CSP {csp-v}.
